...
# ADD THESE GLOBALS (after existing globals)
owner_videos: Dict[str, List[str]] = {}  # Anime → [file_id1, file_id2]
group_welcome_types: Dict[int, Dict[str, str]] = {}  # group → {Anime: "dm"}
video_types_disabled: Set[int] = set()
...
COMMAND_PATTERNS = {
    ...
    "auto_approve": r"/(?:!)?auto_approve(?:\s+(on|off))?",
        # ADD THESE 4 LINES INSIDE the dictionary:
    "add_welcome_video": r"/(?:!)?add_welcome_video\s+(.+)",
    "r_video_type": r"/(?:!)?r_video_type\s+(\w+)",
    "r_welcome_video": r"/(?:!)?r_welcome_video\b",
    "setwelcometype": r"/(?:!)?setwelcometype\s+(.+?)\s+(group|dm|disable)",
    "help": r"/(?:!)?(help|commands)\b",
}

# ─── CHECKPOINT 4 COMPLETE (Lines 601-800) ───────────────────────────────────

# PASTE ALL 5 NEW FUNCTIONS HERE (from my previous message)
async def add_welcome_video_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id if update.effective_user else 0
    if user_id != OWNER_ID:
        return
    reply_msg = update.message.reply_to_message
    if not reply_msg or not reply_msg.video:
        await context.bot.send_message(
            ...
            "👑 Reply to VIDEO with: <code>/add_welcome_video Anime City Nature</code>",
            parse_mode=ParseMode.HTML
        )
        return
    video_id = reply_msg.video.file_id
    match = re.match(r"/(?:!)?add_welcome_video\s+(.+)", update.message.text or "")
    if not match:
        await context.bot.send_message(update.effective_chat.id, "❌ Types required!")
        return
    types = [t.strip() for t in match.group(1).split()]
    for video_type in types:
        if video_type not in owner_videos:
            owner_videos[video_type] = []
        owner_videos[video_type].append(video_id)
    await context.bot.send_message(
        update.effective_chat.id,
        f"✅ Added video to types: {', '.join(types)}\n📹 Total: {len(owner_videos.get(types[0], []))} videos"
    )
    logger.info("Owner added video", types=types, video_id=video_id)
async def remove_video_type_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id if update.effective_user else 0
    if user_id != OWNER_ID: return
    match = re.match(r"/(?:!)?r_video_type\s+(\w+)", update.message.text or "")
    if not match:
        await context.bot.send_message(update.effective_chat.id, "👑 <code>/r_video_type Anime</code>", parse_mode=ParseMode.HTML)
        return
    video_type = match.group(1)
    if video_type in owner_videos:
        del owner_videos[video_type]
        await context.bot.send_message(update.effective_chat.id, f"✅ Removed type: {video_type}")
    else:
        await context.bot.send_message(update.effective_chat.id, f"❌ Type '{video_type}' not found")
async def remove_last_welcome_video_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id if update.effective_user else 0
    if user_id != OWNER_ID: return
    if not owner_videos:
        await context.bot.send_message(update.effective_chat.id, "❌ No videos to remove")
        return
    last_type = list(owner_videos.keys())[-1]
    if owner_videos[last_type]:
        removed_id = owner_videos[last_type].pop()
        await context.bot.send_message(update.effective_chat.id, f"✅ Removed from {last_type}\n📹 <code>{removed_id[:20]}...</code>", parse_mode=ParseMode.HTML)
    else:
        del owner_videos[last_type]
        await context.bot.send_message(update.effective_chat.id, f"✅ Cleared: {last_type}")
async def set_welcome_type_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    chat_id = update.effective_chat.id
    user_id = update.effective_user.id if update.effective_user else 0
    if update.effective_chat.type == "private":
        await context.bot.send_message(chat_id, "❌ Use in group")
        return
    is_admin = await is_group_admin_or_owner(context.application, chat_id, user_id)
    if not is_admin:
        await context.bot.send_message(chat_id, "❌ Admins only")
        return
    match = re.match(r"/(?:!)?setwelcometype\s+(.+?)\s+(group|dm|disable)", update.message.text or "")
    if not match:
        await context.bot.send_message(chat_id, "ℹ️ <code>/setwelcometype Anime dm</code>", parse_mode=ParseMode.HTML)
        return
    types_str, target = match.groups()
    types = [t.strip() for t in types_str.split()]
    target = target.lower()
    if target == "disable":
        video_types_disabled.add(chat_id)
        group_welcome_types.pop(chat_id, None)
        await context.bot.send_message(chat_id, "✅ Videos DISABLED")
        return
    if target not in ["group", "dm"]:
        await context.bot.send_message(chat_id, "❌ 'group', 'dm', or 'disable'")
        return
    if chat_id not in group_welcome_types: group_welcome_types[chat_id] = {}
    for t in types: group_welcome_types[chat_id][t] = target
    video_types_disabled.discard(chat_id)
    await context.bot.send_message(chat_id, f"✅ {', '.join(types)} → {target.upper()}")
async def send_enhanced_dm_welcome(application: Application, user_id: int, group_id: int, user: Any) -> bool:
    if group_id in video_types_disabled: return await send_dm_welcome(application, user_id, group_id, user)
    group_types = group_welcome_types.get(group_id, {})
    dm_types = [t for t,target in group_types.items() if target == "dm"]
    if not dm_types: return await send_dm_welcome(application, user_id, group_id, user)
    video_type = random.choice(dm_types)
    video_id = get_random_video_for_type(video_type)
    if not video_id: return await send_dm_welcome(application, user_id, group_id, user)
    caption = await build_default_welcome_caption(application, group_id, user_id, user.first_name, user.last_name)
    try:
        await application.bot.send_video(user_id, video_id, caption=caption, parse_mode=ParseMode.HTML)
        return True
    except: return await send_dm_welcome(application, user_id, group_id, user)

async def send_enhanced_group_welcome(application: Application, group_id: int, user: Any):
    if group_id in video_types_disabled:
        await send_group_welcome_in_group(application, group_id, user)
        return
    group_types = group_welcome_types.get(group_id, {})
    group_types_list = [t for t,target in group_types.items() if target == "group"]
    config = group_welcome_config.get(group_id)
    if not config or not config.media or config.media.media_type != "video":
        await send_group_welcome_in_group(application, group_id, user)
        return
    if not group_types_list:
        await send_group_welcome_in_group(application, group_id, user)
        return
    video_type = random.choice(group_types_list)
    video_id = get_random_video_for_type(video_type)
    if video_id:
        config.media.file_id = video_id
        try:
            rules_link = group_rules_link.get(group_id)
            chat = await application.bot.get_chat(group_id)
            await send_from_config(application, group_id, config, user, chat, rules_link)
...
async def chat_join_request_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        dm_sent = await send_enhanced_dm_welcome(context.application, user_id, group_id, req.from_user)
        if not dm_sent:
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if msg.new_chat_members:
        ...
        for member in msg.new_chat_members:
            ...
            # Group welcome
            await send_enhanced_group_welcome(context.application, group_id, member)
            
            # DM welcome
            dm_sent = await send_enhanced_dm_welcome(context.application, member.id, group_id, member)
            if not dm_sent:
...
async def command_dispatcher(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for cmd_name, pattern in COMMAND_PATTERNS.items():
        if re.match(pattern, text):
            ...
            elif cmd_name == "add_welcome_video":
                await add_welcome_video_command(update, context)
            elif cmd_name == "r_video_type":
                await remove_video_type_command(update, context)
            elif cmd_name == "r_welcome_video":
                await remove_last_welcome_video_command(update, context)
            elif cmd_name == "setwelcometype":
                await set_welcome_type_command(update, context)
